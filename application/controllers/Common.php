...
class Common extends MY_Controller
{
    ...
    public function api_calc_leave()
    {
        ...
        $list = $this->common_model->selectAll('employee', array('status != ' => 'resigned', 'status != ' => 'noticeperiod'), 'employee_id');
        //pr($list);
        if (!empty($list)) {
    ...
    public function api_leave_reminder()
    {
        ...
        if (!empty($employees)) {
            ...
            $dates = array_filter($dates, function ($date) {
                return !in_array(date("D", strtotime($date)), ['Sun']);
            });
    ...
    //function for auto login based on the cookie in browser
    public function autologin() {

        //setcookie('email', 'harish@ksofttechnologies.com', time() + (86400 * 30), "/");
        ...
        $exist = $this->common_model->selectOne('employee', array('email' => trim($email)), 'employee_id,code,password,designation_id,department_id,status,name,role_id,special_role,email');
    ...
    public function set_cookie() {
        $curl = curl_init();
        curl_setopt_array($curl, [
            CURLOPT_URL => "https://mis.ksofttechnologies.com/api/get-cookie",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
    ...
    function testemail() {
        ...
        $mail->SetFrom('krish@ksofttechnologies.website', 'Ksoft Technologies');
        $mail->addAddress('harish@ksofttechnologies.com', 'Harish');

        $attachment = realpath(__DIR__ . "/../../assets/Ksoft_Technologies_Presentation.pdf");
        $pdfname = 'Ksoft_Technologies_Presentation.pdf';

        if (!file_exists($attachment)) {
            die("PDF file not found: $attachment");
        }

        $fileContent = file_get_contents($attachment);

        $mail->addStringAttachment($fileContent, $pdfname, 'base64', 'application/pdf');

        $mail->IsHTML(true);

        $mail->Subject = 'Testdgdgdgdg';
        $mail->Body    = 'Testgdgdgdg';
    ...
    function test_leave() {
		...
		$leave = $this->leave_model->leave_by_id('228de9ca-ba6c-4aec-93c5-02fefc13a527');
		pr($leave);

        leaveApprovedEmail(5,15,'228de9ca-ba6c-4aec-93c5-02fefc13a527');
